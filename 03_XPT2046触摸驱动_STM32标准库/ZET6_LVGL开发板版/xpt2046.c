...
//	/* 判断得到的值，是否在取值范围之内 */
//	if((*xValue > Raw_x2) || (*xValue < Raw_x1)
//		|| (*yValue > Raw_y2) || (*yValue < Raw_y1))
//	{
//		return 0xFF;
//	}
//	printf("xValue: %d, yValue: %d\n \r", *xValue, *yValue);
	/*TOUCH_X_MAX-TOUCH_X_MIN;*/
	uint16_t xTemp = Raw_x2 - Raw_x1; /* raw 范围 */
	uint16_t yTemp = Raw_y2 - Raw_y1; /* raw 范围 */

	/* 每个 raw 单位对应多少像素（像素/原始单位） */
	float xSlope = (float)LD_HOR_RES_MAX / (float)xTemp; /* X轴缩放 */
	float ySlope = (float)LD_VER_RES_MAX / (float)yTemp; /* Y轴缩放 */

	/* 把 raw_min 映射到像素 0 的偏移量 = -raw_min * slope */
	int16_t x_offset = 0 - (uint16_t)(Raw_x1 * xSlope);
	int16_t y_offset = 0 - (uint16_t)(Raw_y1 * ySlope);

	*xValue = (uint16_t)(*xValue * xSlope) + x_offset;
	*yValue = (uint16_t)(*yValue * ySlope) + y_offset;
